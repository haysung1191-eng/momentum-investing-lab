...
OUTPUT_DIR = ROOT / 'backtests' / 'global_flow_observer_first_model_backtest'

UNIVERSE = ['SPY', 'EFA', 'EEM', 'IEF', 'TLT', 'GLD', 'PDBC', 'BIL']
ONE_WAY_COST_BPS = 10
ONE_WAY_COST_RATE = ONE_WAY_COST_BPS / 10000.0
REGIME_WEIGHTS = {
    'Risk-On': {'SPY': 0.40, 'EFA': 0.20, 'EEM': 0.20, 'GLD': 0.10, 'PDBC': 0.10},
    'Risk-Off': {'IEF': 0.50, 'TLT': 0.30, 'BIL': 0.20},
    'Inflation Tilt': {'GLD': 0.35, 'PDBC': 0.35, 'EEM': 0.15, 'IEF': 0.15},
    'Duration Bid': {'TLT': 0.40, 'IEF': 0.40, 'BIL': 0.20},
    'Mixed': {'SPY': 0.20, 'EFA': 0.10, 'EEM': 0.10, 'IEF': 0.20, 'TLT': 0.10, 'GLD': 0.15, 'PDBC': 0.15},
}
...
def build_target_weights(close: pd.DataFrame, schedule: pd.DataFrame) -> pd.DataFrame:
    ...
    target = target.loc[first_exec:].copy()
    return target
    ...
    out = pd.DataFrame(
        {
            ...
            'TurnoverOneWay': (costs / ONE_WAY_COST_RATE).replace([pd.NA], 0.0).values,
