...
def blend_strategy_results(
    ...
    cagr = float(out["nav"].iloc[-1] ** (1 / years) - 1)
    hwm = out["nav"].cummax()
    mdd = float((out["nav"] / hwm - 1).min())
...
def build_market_matrices(base: str, market: str, max_files: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for f in tqdm(files, desc=f"Load {market} prices"):
        ...
        df["traded_value"] = pd.to_numeric(df["close"], errors="coerce").fillna(0.0) * pd.to_numeric(df["volume"], errors="coerce").fillna(0.0)
